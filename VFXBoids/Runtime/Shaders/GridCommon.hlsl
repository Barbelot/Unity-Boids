...
uint2 GetGridPosition(float2 pos, float2 centerBox, float2 sizeBox)
{
    ...
    uint2 uPos = floor(normalizedPos * float2(GRID_DIM_X, GRID_DIM_Y));
    return uPos;
}

uint GetGridLinearIndex(uint2 uPos)
{
    uPos = min(uPos, uint2(GRID_DIM_X, GRID_DIM_Y));
    return uPos.x * GRID_DIM_Y + uPos.y;
}
